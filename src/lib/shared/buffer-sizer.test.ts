...
describe('BufferSizer buffer adjustment', () => {
	test.each(cases)('$name', (c) => {
		...
		recordMissables(bufferSizer, c.missables, startTime, startTime + adjustBufferSizeIntervalMs);
...
function recordMissables(
	...
	for (let i = 0; i < missables.length; i++) {
		...
		for (let j = 0; j < count; j++) {
			const now = i === missables.length - 1 && j === count - 1 ? timeOfLastRecord : startTime;
			bufferSizer.recordMissable(now, missed, bufferNeededMs, new LogContext('error'));
		}
	}
}
